...
int k=0;
void print_path(struct graph* graph,int s,int v,int *p)
{
    if(v==s)
    {
        printf("%d",s);
    ...
    {
        print_path(graph,s,p[v],p);
        printf("%d",v);
    }
}
int visit_dfs(int s,struct graph* graph,int *visited,int *st,int *ft,int *time,int *p,int v)
{
    ...
    while(temp!=NULL)
    {
        if(visited[temp->dest]==0)
        {
            ...
            visit_dfs(temp->dest,graph,visited,st,ft,time,p,v);
        }
        else if(visited[temp->dest]==1)
        {
           print_path(graph,temp->dest,s,p);
           k++;
           printf("\n");
        }
        temp=temp->next;
...
int dfs(struct graph* graph,int v,int *visited,int *st,int *ft,int *time,int *p)
{
    int i;
    for(i=0;i<v+1;i++)
    {
    ...
    for(i=1;i<v+1;i++)
    {
        if(visited[i]==0)
        {
            visit_dfs(i,graph,visited,st,ft,time,p,v);
        }
    }
}
...
int main()
{
    ...
    int time=0;
    int p[V+1];
    dfs(graph,V,visited,st,ft,&time,p);
    printf("Number of cycles is %d",k);
    return 0;
}
